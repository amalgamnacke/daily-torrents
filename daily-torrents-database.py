...
class DailyTorrentsDatabase:
	def __init__(self):
		...
		try:
			cur = con.cursor()
			cur.execute("CREATE TABLE IF NOT EXISTS movies(title TEXT, magnet_url TEXT)")
		except:
			raise
		con.close()
	...
	def insertMovie(self, title, magnet_url):
		con = self.get_DB_connection()
		cur = con.cursor()

		cur.execute("INSERT INTO movies VALUES(?,?)", (title, magnet_url))
